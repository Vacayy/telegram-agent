...
### 추가된 기능
Tool Registry의 사용량 추적 기능을 사용자가 조회할 수 있도록 명령어 추가.

### 사용 방법
1. **명령어**: `/usage`
2. **자연어**: "사용량 알려줘", "얼마나 썼어?", "비용 확인"
...
도구별 사용량
• web_search: 5회 (500 in / 2,000 out) $0.0011
• translate: 3회 (1,000 in / 800 out) $0.0000
...
| 파일 | 변경 내용 |
|------|----------|
| `src/agent.py` | UserIntent.USAGE 추가, 프롬프트에 usage 의도 추가 |
| `src/bot.py` | `/usage` 명령어 핸들러, format_usage_report() 함수 |

### 한계
- 현재 세션(봇 프로세스) 동안의 사용량만 추적
- 봇 재시작 시 초기화됨
- 향후 DB 저장으로 영구 추적 가능
...
### 배경
사용자의 질문에 대해 다양한 관점에서 심층 분석을 제공하기 위해 AI 토론 기능을 도입.
단일 AI 응답의 한계(편향, 단일 관점)를 극복하고 균형 잡힌 인사이트를 제공.

### 기능 개요
3명의 AI 페르소나(낙관론자, 비관론자, 중립자)가 5라운드에 걸쳐 토론을 진행하고, 최종 리포트를 생성.
...
```
┌─────────────────────────────────────────────────────────────┐
│  [1] Round 1: 입장 발표 (Opening Statements)                 │
│      - 각 페르소나가 주제에 대한 입장과 핵심 논거 제시        │
├─────────────────────────────────────────────────────────────┤
│  [2] Round 2: 교차 질문 (Cross-Examination)                  │
│      - 다른 참가자의 논리적 허점이나 불확실한 가정 지적       │
├─────────────────────────────────────────────────────────────┤
│  [3] Round 3: 반박 및 방어 (Rebuttal & Defense)              │
│      - 받은 질문에 답변하고 자신의 입장 방어/보강             │
├─────────────────────────────────────────────────────────────┤
│  [4] Round 4: 입장 조율 (Position Refinement)                │
│      - 타당한 부분 인정, 입장 수정/보완, 공통점 도출          │
├─────────────────────────────────────────────────────────────┤
│  [5] Round 5: 최종 정리 (Closing Statements)                 │
│      - 토론을 통해 도달한 최종 결론과 핵심 인사이트           │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│  [6] 최종 리포트 생성                                        │
...
| 파일 | 역할 |
|------|------|
| `src/debate/__init__.py` | 모듈 진입점, 주요 클래스 export |
| `src/debate/persona.py` | 페르소나 정의 및 동적 생성 (PersonaGenerator) |
| `src/debate/orchestrator.py` | 토론 진행 총괄 (DebateOrchestrator) |
| `src/bot.py` | `/debate` 명령어 핸들러 추가 |

### 핵심 기능

**1. 동적 페르소나 생성**
- Gemini 2.0 Flash가 주제에 맞는 3명의 전문가 구성
- 예: "비트코인" 주제 → 암호화폐 시장 전문가, 리스크 분석가, 퀀트 애널리스트
- 기본 템플릿: 낙관론자(🐂), 비관론자(🐻), 중립자(⚖️)

**2. 병렬 처리**
- 각 라운드에서 3명의 페르소나가 동시에 응답 생성
- `asyncio.gather()` 활용으로 처리 시간 단축

**3. ExpandableBlockQuote 활용**
- 완료된 라운드는 접힌 상태로 표시
- 현재 라운드는 펼쳐서 표시
- 사용자가 탭하여 이전 라운드 확인 가능

**4. 최종 리포트**
- 합의점: 세 참가자 모두 동의한 내용
- 쟁점: 의견이 갈린 핵심 포인트
- 종합 결론: 균형 잡힌 최종 판단
- 실행 제안: 사용자를 위한 구체적 행동 지침
- 통계: 토론 시간, 총 발언 수
...
| 항목 | 모델 | 비용 |
|------|------|------|
| 페르소나 생성 | Gemini 2.0 Flash | 무료 |
| 5라운드 토론 (3명×5) | Gemini 2.0 Flash | 무료 |
| 리포트 생성 | Gemini 2.0 Flash | 무료 |
| **합계** | | **$0.00** |
