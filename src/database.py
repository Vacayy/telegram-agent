...
async def get_all_messages_as_context(user_id: int, limit: int = 20) -> str:
    ...
    /list 명령어와 동일한 순서(최신순)와 번호를 부여하여
    "1번 메시지", "2번 메시지" 등의 참조가 정확히 동작하도록 합니다.
    """
    ...
    context_parts = []
    # messages는 최신순(DESC) - /list와 동일한 순서
    # 1번 = 가장 최근 메시지
    ...
    for idx, msg in enumerate(messages, 1):
        ...
        # 번호를 명시적으로 부여 (1번 = 가장 최근)
        if idx == 1:
            context_parts.append(
                f"[{idx}번 메시지 - 가장 최근] {source} ({time_str})\n{msg['content']}"
            )
        else:
            context_parts.append(f"[{idx}번 메시지] {source} ({time_str})\n{msg['content']}")
